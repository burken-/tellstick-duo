...
SET(XC8_PATH "/opt/microchip/xc8/v1.33/bin/xc8" CACHE STRING "Path to xc8 compiler")
...
FUNCTION(ADD_HEX _project_name _default_chip)
	...
	FOREACH(_file ${ARGN})
		...
		IF(_ext STREQUAL ".c")
			ADD_CUSTOM_COMMAND(OUTPUT ${_p1_path}
				COMMAND ${XC8_PATH} -Q ${_definitionlist} --chip=${CHIP} ${CMAKE_C_FLAGS} --errformat='%f:%l: error[%n]: %s' --outdir=${CMAKE_BINARY_DIR}/${_dir} ${_include_arg} --pass1 "${_abs_file}"
				DEPENDS ${_abs_file}
	...
	ADD_CUSTOM_COMMAND(OUTPUT ${_hex_file}
		COMMAND ${XC8_PATH} -Q --chip=${CHIP} ${CMAKE_EXE_LINKER_FLAGS} ${_p1_sources} --outdir=${CMAKE_BINARY_DIR} -O${_hex_file}
		DEPENDS ${_p1_sources}
	...
	OPTION(FLASH_PICKIT3 FALSE "Flash the hex using PICkit 3 after a successful build")
	IF(FLASH_PICKIT3)
		ADD_CUSTOM_COMMAND(TARGET ${_project_name} POST_BUILD
			COMMAND /opt/microchip/mplabx/mplab_ipe/ipecmd.sh -P${CHIP} -F${CMAKE_CURRENT_BINARY_DIR}/${_hex_file} -TPPK3 -OL -M
			DEPENDS ${_hex_file}
